...
" Lowest-precedence generic patterns {{{2
"
" These are the most generic match rules for identifiers and built-in types.
" The same patterns will be used to provide more specific, context-dependent
" match groups for the same identifiers later. Since these need to have the
" lowest match precedence, they need to be defined earliest (see
" syn-priority).
" 
" Makes a distinction between identifier types in line with RFC 430, except it
" also matches aValueIdentifier
" https://rust-lang.github.io/api-guidelines/naming.html
syn match     rustIdentifierValueLevel "_*[a-z]\w*"
syn match     rustIdentifierTypeLevel  "\(_*[A-Z]\+[_a-z0-9]*\)\+"
syn keyword   rustType        isize usize char bool u8 u16 u32 u64 u128 f32
syn keyword   rustType        f64 i8 i16 i32 i64 i128 str Self


" This rustPath match wants to do more than match the generic, official syntax
" for a path. This is because we often want to match the final segment of the
" path specially. For example:
"
" (a) This module-scoped function will want to match x::y:: as the modPath
"     while the z() will be matched as a rustFuncCall.
"     x::y::z() 
" (b) In contrast, it can be useful to style the final segment in a use
"     modPath different from the rest of the modPath because it acts as a
"     namespace, with an importance beyond the use stanza that the path prefix
"     lacks. Here we want to match z and Z both as types.
"     use x::y::z;
"     use x::y::{Z};
syn match     rustPath        "\(::\)\?\(\w\+::\)\+\([a-z][a-z0-9]*;\)\?" keepend
            \ contains=rustPathSep,rustPathSpecialSegment,rustPathIdentifierTypeLevel,rustPathIdentifierValueLevel,rustPathFinalIdentifierValueLevel
syn match     rustPathSep                  "::" contained
syn match     rustPathIdentifierValueLevel "_*[a-z]\w*" contained
syn match     rustPathIdentifierTypeLevel  "\(_*[A-Z][_a-z0-9]*\)\+" contained
syn match     rustPathSpecialSegment       "\(super\|self\|Self\|[$]\?crate\)" contained
syn match     rustPathFinalIdentifierValueLevel "_*[a-z]\w*;" contained
...
syn region    rustEnumStructEntry matchgroup=rustEnumVariantDecl skip="[^}]" start="_*[A-Z]\w*\s*{" end="}" 
...
syn keyword   rustStructMemberType        isize usize char bool u8 u16 u32 u64 u128 f32 contained
syn keyword   rustStructMemberType        f64 i8 i16 i32 i64 i128 str Self contained
...
syn keyword   rustStructure struct enum nextgroup=rustIdentifierTypeLevel skipwhite skipempty contained
...
" syn match     rustTypeScope   "[A-Z]\w*\(::[^<]\)\@="

" If foo::bar changes to foo.bar, change this ("::" to "\.").
" If foo::bar changes to Foo::bar, change this (first "\w" to "\u").
" syn match     rustModPath     "[a-z][a-z0-9]*\(::[^<]\)\@="
" syn match     rustEnumVariantMatch "::[A-Z]\w*("hs=s+2,ms=s+2,he=e-1,me=e-1

syn match     rustFuncCall    "[a-z]\w*("he=e-1,me=e-1
...
hi def link rustIdentifierTypeLevel rustType
hi def link rustIdentifierValueLevel rustIdentifier
hi def link rustPathSep                  rustOperator
hi def link rustPathSpecialSegment       rustKeyword
hi def link rustPathIdentifierTypeLevel  rustIdentifierTypeLevel
hi def link rustPathIdentifierValueLevel rustIdentifierValueLevel
hi def link rustPathFinalIdentifierValueLevel rustIdentifierTypeLevel
...
hi def link rustTypeScope     rustType
